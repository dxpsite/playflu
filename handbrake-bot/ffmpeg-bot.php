<?php
	...
	$mediadrop_video_dir="/home/crud/upl2/uploads/"; // Mediadrop Media directory, note the trailing slash
	...
	$handbrake_command = "ffmpeg ";
	//$handbrake_preset = " --preset=\"Universal\" ";
	...
	$handbrake_preset = " -b:v 2048k -s 720x576 -vcodec libx264 -preset medium -acodec aac -ar 44100 -ab 96k -threads 1 ";
	$new_extension = "mp4";
...
function getTranscodeCommandString($handbrake_command, $handbrake_preset, $new_extension, $mediadrop_video_dir, $srcFile, $destFile) {
	return $handbrake_command." -i ".$mediadrop_video_dir.$srcFile.$handbrake_preset." ".$mediadrop_video_dir.$destFile;
}
...
function mark_new_media_as_encoded($db_name, $new_media_file_name, $mediadrop_video_dir, $media_id,$media_size) {
	   ...
	   // $fileSize = filesize($file_path.$new_file);
	    ...
	    rename($mediadrop_video_dir.$new_media_file_name, $mediadrop_video_dir.$new_id['id']."testbed-".$new_media_file_name );
